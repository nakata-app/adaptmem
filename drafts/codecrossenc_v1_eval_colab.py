# Colab cell: CodeCrossEnc-v1 rerank eval on CodeSearchNet python full test (22k)
#
# Pipeline:
#   1. FT-Code-5000 bi-encoder retrieve top-20 candidates per query
#   2. CodeCrossEnc-v1 cross-encoder rerank those 20
#   3. Compute R@1, R@5, R@10, MRR on full test set
#
# Compare against:
#   - FT-Code-5000 alone:           R@1 = 0.926, MRR = 0.952
#   - FT-Code-5000 + ms-marco rerank: R@1 ~ 0.90  (negative transfer)
#   - FT-Code-5000 + BGE rerank:    R@1 ~ 0.90  (negative transfer)
#
# Hedef: R@1 >= 0.93  -> encoder + reranker axes compose ediyor
#         R@1 <  0.93 -> trivial overfit, hard negative mining v2 lazim
#
# Calistirma yeri: Colab T4. Tahmini sure: ~15-20dk (22k queries, 22k corpus, top-20 rerank).
...
# ---- Cell 3: Load test set (same protocol as codesearchnet_eval.py) ----
import time, json
...
t1 = time.time()
sims = query_emb @ corpus_emb.T  # cosine (both normalised)
# argpartition for top-K (much faster than full argsort on 22k x 22k)
...
# Bi-encoder alone metrics (sanity check, should be ~0.926 R@1)
...
for qi in range(len(queries)):
    ...
    if len(pos_arr) == 0:
        continue  # truth outside top-20, contributes 0 to all
    pos = int(pos_arr[0])
    if pos == 0:  bi_r1  += 1
    if pos < 5:   bi_r5  += 1
    if pos < 10:  bi_r10 += 1
...
# Build (query, candidate_body) pairs for all queries x top-20
t2 = time.time()
pairs = []
for qi in range(len(queries)):
    q = queries[qi]
...
# Score in chunks (avoid OOM on 22k * 20 = 440k pairs)
CHUNK = 4096
...
with out_path.open('w') as f:
    for qi in range(len(queries)):
        ...
        pos_arr = np.where(ranked == truth_idx_arr[qi])[0]
        if len(pos_arr) == 0:
            rank = TOP_K + 1  # truth never reranked (outside top-20)
        else:
...
# ---- Decision gate ----
if summary['reranked']['R@1'] >= 0.93:
    print('GATE: PASS, R@1 >= 0.93. Cross-harness probe (mempalace C-AST cs800) sirada.')
else:
    print(f"GATE: FAIL, R@1 = {summary['reranked']['R@1']:.4f} < 0.93. Hard negative mining v2 plani.")
