...
2. **Hybrid_v4 + FT-300 went up: R@1 +0.034, R@5 +0.008, R@10 +0.002** relative to the Apr 28 run. This is consistent with the v3.3.5 BM25 hybrid rerank fix, the rerank pass is FT-300-encoder-aware now in a way it wasn't before, and the encoder layer's lift composes with the fixed rerank rather than getting clipped by it. The orthogonality framing in `adaptmem-orthogonal-layers.md` strengthens rather than weakens at v3.3.5.
...
6. **Link to `adaptmem-orthogonal-layers.md`?**, jphein wrote it on his fork, not upstream. If you want to link, use `https://github.com/jphein/mempalace/blob/main/docs/research/adaptmem-orthogonal-layers.md`. Optional; I'd lean toward not linking unless he surfaces it himself in the thread.
