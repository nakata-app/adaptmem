...
- [ ] **PyPI release.** Tag `v0.6.0`, populate `PYPI_API_TOKEN` repo secret, the gated `release.yml` step publishes the wheel. After this, downstream consumers (e.g. `claimcheck`) can drop their editable-sibling install in favour of `dependencies = ["adaptmem>=0.6"]`.
...
- [x] **PoC implementation.**
  ...
  - **Python clients**: `claimcheck.Pipeline.from_daemon`. Shipped with mock-server tests.
...
- [x] **Integration tests.** FastAPI TestClient in adaptmem tests; tokio mockable client in metis tests. Cross-process end-to-end on Linux is the next gate (Mac/Py3.14 deadlock blocks local live smoke).
